...
namespace NeuralNetworks
{
    ...
    public class BackpropNetwork
    {
        ...
        List<double[]> inputs; // training set inputs
        List<double[]> outputs; // training set outputs
        ...
        public void Backprop(double[] x, double[] target)
        {
            ...
            for (int i = 0; i <NeuralNet.WeightMat2.GetLength(0); i++)
            {
                for (int j = 0; j < NeuralNet.WeightMat2.GetLength(1); j++)
                {
                    ...
                    //MomentumMat2[i, j] = Eta * outputDeltas[i] * h[j] + MomentumMat2[i, j] * Momentum;
                    //NeuralNet.WeightMat2[i, j] += MomentumMat2[i, j];
                }
            }
            ...
            //Parallel.For(0, NeuralNet.WeightMat1.GetLength(0), i =>
            for (int i = 0; i < NeuralNet.WeightMat1.GetLength(0); i++)
            {
                for (int j = 0; j < NeuralNet.WeightMat1.GetLength(1); j++)
                {
                    ...
                    //MomentumMat1[i, j] += Eta * hiddenDeltas[i] * inputWithBias[j] + MomentumMat1[i, j] * Momentum;
                    //NeuralNet.WeightMat1[i, j] += MomentumMat1[i, j];
                }
            }
        }
        ...
        public List<double[]> TrainNetwork()
        {
            ...
            for (int ep=0;ep<Episodes;ep++)
            {
                ...
                if (ep == 0 || (ep + 1) % 100 == 0)
                {
                    Console.WriteLine("Episode {0}", ep + 1);
                }
                */
